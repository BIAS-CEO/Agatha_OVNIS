...
# VERSION: Opcon Ready v5.8 (Metricas Globales Inteligentes)
# OPERADOR: DIR-74
# ====================================================================
...
with st.status("Inicializando Motor de Analisis Conductual Predictivo...", expanded=True) as status_boot:
    ...
    def simular_coordenadas(df):
        ...
        # Saneamiento previo para evitar KeyError
...
with col_filtros:
    ...
    if not df_maestro.empty and 'AÑO' in df_maestro.columns:
        ...
        # LOGICA DE METRICAS GLOBALES VS FILTRADAS
        filtros_activos = (sel_anio != "TODOS") or (sel_mes != "TODOS") or (sel_dia != "TODOS") or \
                         (sel_hora != "TODAS") or (sel_forma != "TODOS") or (sel_pais != "TODOS")
    else:
...
# Si no hay filtros activos, se muestra el total de df_maestro. De lo contrario, se segmenta por df_filtrado.
df_target = df_filtrado if filtros_activos else df_maestro

m1, m2, m3 = st.columns(3)
label_segmento = " (TOTALES)" if not filtros_activos else " (FILTRADOS)"

m1.metric(f"REGISTROS ACTIVOS{label_segmento}", f"{len(df_target):,}")
m2.metric(f"TIPOLOGIA PREDOMINANTE{label_segmento}", df_target['FORMA'].mode().iloc[0] if not df_target.empty else "N/A")
m3.metric(f"ZONAS DE INTERES (NODOS){label_segmento}", f"{len(df_target['CIUDAD'].unique()) if not df_target.empty else 0:,}")

# --- REGISTROS FORENSES ---
with st.expander(f"REGISTROS FORENSES ({len(df_filtrado)} Activos)", expanded=True):
    if not df_filtrado.empty:
        # Aquí siempre mostramos lo filtrado para que el usuario sepa qué está viendo en la tabla
...
st.divider()
st.markdown("**Nota de Confidencialidad:** Informe generado por Motor de Análisis Conductual Predictivo. Nivel de acceso 5 requerido para exportación.").block-container { 
    padding-top: 2rem !important; 
    padding-bottom: 2rem !important; 
    max-width: 98% !important; 
}
